...
class AutocertoParser(BaseParser):
    ...
    def parse(self, data: Any, url: str) -> List[Dict]:
        ...
        for v in veiculos:
            ...
            parsed = self.normalize_vehicle({
                "id": v.get("idveiculo"), "tipo": v.get("tipoveiculo"), "titulo": None, "versao": ((v.get('modelo', '').strip() + ' ' + ' '.join(re.sub(r'\b(\d\.\d|4x[0-4]|\d+v|diesel|flex|gasolina|manual|automático)\b', '', v.get('versao', ''), flags=re.IGNORECASE).split())).strip()) if v.get("versao") else (v.get("modelo", "").strip() or None),
